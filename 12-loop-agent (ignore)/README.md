# facebook Post Generator Loop Agent

This example demonstrates the use of a Sequential and Loop Agent pattern in the Agent Development Kit (ADK) to generate and refine a facebook post.

## Overview

The facebook Post Generator uses a sequential pipeline with a loop component to:
1. Generate an initial facebook post
...
`facebookPostGenerationPipeline` - A SequentialAgent that orchestrates the overall process:
1. First runs the initial post generator
2. Then executes the refinement loop

### Initial Post Generator

`InitialPostGenerator` - An LlmAgent that creates the first draft of the facebook post with no prior context.
...
"Generate a facebook post about what I've learned from @aiwithbrandon's Agent Development Kit tutorial."
